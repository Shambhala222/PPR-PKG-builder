...
public sealed class ExtendedApp : LibProsperoPkg.Gui.App
{
    public override void OnFrameworkInitializationCompleted()
    {
        ...
        window.ClearValue(Window.TitleProperty); window.Title="FPKG Builder 0.8.2 – macOS";
        ...
        original[1].ClearValue(HeaderedContentControl.HeaderProperty); original[1].Header="Inspect";
        ...
        foreach(var text in window.GetLogicalDescendants().OfType<TextBlock>().Where(t=>t.Text=="v0.6.5.2").ToArray()) {text.ClearValue(TextBlock.TextProperty);text.Text="v0.8.2";}
...
public sealed class SdkTab : UserControl
{
    ...
    public SdkTab(Window owner)
    {
        ...
        source.PlaceholderText="Game/project folder"; output.PlaceholderText="Choose an output folder";temporary.PlaceholderText="Optional (defaults to output folder)";reference.PlaceholderText="Optional (base PKG for a patch)";package.PlaceholderText="Existing .pkg for verify/extract";extract.PlaceholderText="Empty folder for extract";
        Row("Source folder",source,()=>ChooseFolder(source));Row("Output folder",output,()=>ChooseFolder(output));Row("Temporary folder",temporary,()=>ChooseFolder(temporary));Row("PKG file",package,ChoosePackage);Row("Extraction folder",extract,()=>ChooseFolder(extract));Row("Reference PKG",reference,ChooseReference,
    ...
    {
        Bitmap? next=null;
        try {var path=Path.Combine(source.Text??"","sce_sys","icon0.png");if(File.Exists(path)){using var stream=File.OpenRead(path);next=Bitmap.DecodeToWidth(stream,280);}} catch { }
    ...
    async Task Extract()
    {
        ...
        await RunSdk("extract", extra=> { extra.Add("--package"); extra.Add(package.Text!); extra.Add("--extract"); extra.Add(extract.Text!); }, success=> { lastFolder=success; });
    }
    async Task Build()
    {

        if(IsBusy)return;
        if(string.IsNullOrWhiteSpace(source.Text)||string.IsNullOrWhiteSpace(output.Text)){ShowMessage("Choose a source folder and output folder first.");return;}
        string packageName=Path.GetFileName((source.Text??"").TrimEnd(Path.DirectorySeparatorChar));
        try {using var param=JsonDocument.Parse(File.ReadAllText(Path.Combine(source.Text!,"sce_sys","param.json")));if(param.RootElement.TryGetProperty("contentId",out var cid)&&!string.IsNullOrWhiteSpace(cid.GetString()))packageName=cid.GetString()!;} catch { }
        ...
        foreach(var arg in new[]{"-I","-B","-u",Path.Combine(runtime,"sdk-runner.py"),"--mode","build","--source",source.Text!,"--output",packageOutput,"--temp",temporary.Text??"","--reference",reference.Text??"","--compression",((int)(compression.Value??7)).ToString(),"--chunk-count",((int)(chunks.Value??100)).ToString(),"--passcode",passcode.Text??""})info.ArgumentList.Add(arg);
    ...
    async Task RunSdk(string mode, Action<System.Collections.Generic.List<string>> extraArgs, Action<string> onSuccess)
    {
        if(IsBusy)return;
        string runtime=Path.GetFullPath(Path.Combine(AppContext.BaseDirectory,"../Resources/SDKRuntime"));
        var info=new ProcessStartInfo(Path.Combine(runtime,"python/bin/python3.12")){UseShellExecute=false,RedirectStandardOutput=true,RedirectStandardError=true,CreateNoWindow=true};
        ...
        try{
            ...
            else if(process.ExitCode==0 && result!=null){status.Text="Done – "+result;progress.IsIndeterminate=false;progress.Value=100;onSuccess(result);}
