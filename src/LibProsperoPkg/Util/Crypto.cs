...
public static class Crypto
{
    ...
    /// the per-block hashes. Uses the OS SHA-3 provider when present, otherwise a managed
    /// Keccak-f[1600] implementation (required on macOS).
    /// </summary>
    public static byte[] Sha3_256(byte[] data) => ProsperoSha3.HashData(data);

    public static byte[] Sha3_256(ReadOnlySpan<byte> data) => ProsperoSha3.HashData(data);
