...
/// <see cref="ComputeBlockDigest(System.ReadOnlySpan{byte})"/> returns. Always available
/// (OS SHA-3 or the managed Keccak implementation).
/// </remarks>
public static class ProsperoPfsDigest
{
    ...
    /// <summary>Always <see langword="true"/>; SHA3-256 is provided by OS or a managed implementation.</summary>
    ...
    public static byte[] ComputeBlockDigest(ReadOnlySpan<byte> uncompressedBlock) =>
        ProsperoSha3.HashData(uncompressedBlock);
    ...
    public static int ComputeBlockDigest(ReadOnlySpan<byte> data, Span<byte> destination) =>
        ProsperoSha3.HashData(data, destination);
        ...
        ReadOnlySpan<byte> blockHashSection)
    {
        ...
        var hash = new ProsperoSha3.Hasher();
        hash.Append(header32);
        hash.Append(shuffleSection);
        hash.Append(boundarySection);
        hash.Append(blockHashSection);
        return hash.Finish();
    }
