...
public static class ProsperoNapsLayout
{
    ...
    /// <summary>
    /// Windows 0.5 / publisher section map: fidx count is <see cref="NapsLayoutCounts.NumFiles"/>
    /// (the last record is the type-0x40 mount sentinel), and CblockInfo is 8-byte aligned
    /// after the u2c table.
    /// </summary>
    public static NapsSectionMap SectionMapPublisher(NapsLayoutCounts counts)
    {
        long pos = 0;
        var header = new NapsSection(pos, HeaderSize, HeaderSize, 1);
        pos += HeaderSize;
    ...
    public static NapsLayoutDocument Parse(ReadOnlySpan<byte> blob, NapsLayoutCounts counts, int fidxEntryCount)
        => Parse(blob, counts, SectionMap(counts, fidxEntryCount));

    /// <summary>
    /// Parse a publisher / Windows 0.5 <c>naps_pkg_layout.dat</c> (NumFiles fidx records, 8-byte
    /// aligned CblockInfo). Use this for unpack; <see cref="Parse(ReadOnlySpan{byte})"/> stays
    /// on this library's writer layout.
    /// </summary>
    public static NapsLayoutDocument ParsePublisherLayout(ReadOnlySpan<byte> blob)
    {
        NapsLayoutCounts counts = DecodeHeader(blob);
        return Parse(blob, counts, SectionMapPublisher(counts));
    }

    private static NapsLayoutDocument Parse(ReadOnlySpan<byte> blob, NapsLayoutCounts counts, NapsSectionMap map)
    {
